...
void widget_update_handle( object_t *obj, event_t *event )
{
	...
	if ( WIDGET(obj)->size_req != 0 && WIDGET(obj)->size_req->owner != 0 )
		widget_size_request_changed( obj );
}
...
void widget_post_init( object_t *widget )
{
	...
	object_addhandler( widget, "destroy", widget_destroy_handle );
	object_addhandler( widget, "update", widget_update_handle );
	object_addhandler( widget, "content_resized", widget_resized_handle );
