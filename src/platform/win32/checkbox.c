...
void cgraphics_checkbox_widget_create( widget_t *widget )
{
	...
	if ( !( hwnd = CreateWindowEx( 0, "BUTTON", 
	                               "",
	                               WS_CHILD | WS_TABSTOP | BS_CHECKBOX | BS_AUTOCHECKBOX,
...
void cgraphics_checkbox_update_checked( checkbox_widget_t *widget )
{
	HWND hwnd = widget->widget.native;
	SendMessage( hwnd, BM_SETCHECK, (widget->checked?BST_CHECKED:BST_UNCHECKED), 0 );
}
