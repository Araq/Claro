...
if env['PLATFORM'] == "darwin":
	...
	conf.env.Append(CCFLAGS=["-D_MAC","-fPIC","-fno-common"], LINKFLAGS=["-framework","Carbon","-framework","Cocoa"])
	excl = '-DNO_CAIRO'
	if no_cairo == 0:
		excl = ''
	...
	conf.env.Append(CCFLAGS=['-DIMAGES_USE_LIBPNG'], LINKFLAGS=['-LC:/msys/1.0/local/lib'], LIBS=['gdi32','ole32'])
	...
	# cairo
	if no_cairo == 0:
		conf.env.Append(CCFLAGS=['-I/usr/local/include/cairo','-I/usr/include/cairo'])
